...
- Add the project id to the TOC — the workflow reads it from there, and there
  are commented placeholders showing where:

  ```
  ## X-Curse-Project-ID: <id>
  ```
- Add the repository secret `CF_API_TOKEN` (CurseForge → Authors → API Tokens).
  The name matters: the packager reads `CF_API_TOKEN`, and a differently-named
  secret produces an upload that silently does not happen.
...
- Create the addon at <https://addons.wago.io>, then add its id to the TOC:
  ...
  Do not commit a made-up id — wrong metadata ships to players and is harder to
  notice than missing metadata.
- Add the repository secret `WAGO_API_TOKEN` (wago.io → account settings → API
  keys).
...
`.github/workflows/release.yml` runs [BigWigsMods/packager](https://github.com/BigWigsMods/packager)
on a release tag. It reads `## Interface` and both project ids out of the TOC,
so no game version and no project id is written down anywhere else.

A store whose id is missing from the TOC is skipped. With neither token
configured the packager runs with `-d`: it builds the zip and proves the addon
packages cleanly, and uploads nothing. So the job is safe to leave switched on
before either project exists, and safe in a fork.

Secrets go in without passing through a terminal history or a chat log:

```
gh secret set CF_API_TOKEN   --repo Yumash/BabelChat   # prompts, hidden input
gh secret set WAGO_API_TOKEN --repo Yumash/BabelChat
```

Token files at the repository root (`.cf`, `.curseforge`, `.wago`, anything
matching `.*token*`) are gitignored, and a test asserts they stay that way —
this repository is public.

The GitHub release itself is made by the `release` job, so `GITHUB_API_TOKEN` is
deliberately not handed to the packager: it would create a second release.
