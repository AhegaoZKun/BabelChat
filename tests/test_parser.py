...
class TestParsePublicChannels:
    """Public channels added alongside the Linux port — they shipped untested."""
    ...
    def test_public_channel_is_parsed(self, raw, expected):
        line = f"2/15 21:30:45.123  [{raw}] Trader-Server: WTS boost 200k"
        msg = parse_line(line)
        assert msg is not None
        assert msg.channel == expected
        assert msg.author == "Trader"
        assert msg.text == "WTS boost 200k"
...
class TestParseEdgeCases:
    ...
    def test_unknown_channel_returns_none(self):
        # Guard the premise: if this name ever lands in _CHANNEL_MAP, fail here
        # instead of letting the case decay into an always-true assert.
        assert "Bloodlust" not in _CHANNEL_MAP
        line = "2/15 21:30:45.123  [Bloodlust] Spammer-Server: WTS boost"
        msg = parse_line(line)
        assert msg is None
    ...
    def test_wow_item_link_filtered(self):
        line = "2/15 21:30:45.123  [Party] Player-Server: |cFFFF8800|Hitem:12345|h[Sword of Truth]|h|r"
        msg = parse_line(line)
        assert msg is None
